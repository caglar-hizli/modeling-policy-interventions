...
class MarkCoregion(Kernel):
    ...
    def output_covariance(self, X, X2):
        W = self.beta0 + self.beta1 * X
        W2 = self.beta0 + self.beta1 * X2
    ...
    def output_variance(self, X):
        ...
        if X.shape[0] == self.kappa.shape[0]:
            B_diag += self.kappa
        return B_diag
...
class HierarchicalMarkCoregion(Kernel):
    ...
    def __init__(
        ...
        *,
        ...
        self.beta0_raw = Parameter(to_default_float(1.0) * tf.ones(num_patients, dtype=tf.float64),
                                   transform=positive())
        # self.beta0_raw.prior = tfp.distributions.Normal(to_default_float(0.0), to_default_float(1.0))
        self.beta1_raw = Parameter(to_default_float(1.0) * tf.ones(num_patients, dtype=tf.float64),
                                   transform=positive())
        # self.beta1_raw.prior = tfp.distributions.Normal(to_default_float(0.0), to_default_float(1.0))
    ...
    def output_covariance(self, X, X2):
        ...
        beta0_vec = tf.reshape(tf.gather(beta0, self.t_pidx), (-1, 1))  # (num_treatments1,)
        beta1_vec = tf.reshape(tf.gather(beta1, self.t_pidx), (-1, 1))  # (num_treatments1,)
        W = beta0_vec + beta1_vec * X
        beta0_vec = tf.reshape(tf.gather(beta0, self.t_pidx2), (-1, 1))  # (num_treatments2,)
        beta1_vec = tf.reshape(tf.gather(beta1, self.t_pidx2), (-1, 1))  # (num_treatments2,)
        W2 = beta0_vec + beta1_vec * X2
