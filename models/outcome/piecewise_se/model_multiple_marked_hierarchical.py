...
class TRSharedMarkedHier(gpf.models.GPR):
    ...
    def __init__(self, data, t, T_treatment, baseline_kernels, treatment_base_kernel, mean_functions, patient_ids,
                 noise_variance=1.0, train_noise=True):
        """
        Implementation of the non-parametric outcome model, y(\tau) = f_b(\tau) + f_a(\tau) + noise
            It has a GP baseline f_b and a GP treatment response f_a.
            The treatment response f_a is further factorized as: f_a(\tau) = \sum_j l(m_meal_j) f_t(\tau - t_meal_j).
            Optimized using the marginal likelihood.

        Np: #patients
        Ni: #glucose observations for individual i
        Mi: #meals for individual i

        :param data:
            (X, Y): Both X and Y are lists of length Np. Each list has elements of (Ni,1) arrays,
            that contains patient-specific outcome time and value measurements.
        :param t:
            (t_meal, m_meal): Both lists of length Np. Each list has elements of (Mi, 2) arrays,
            that contains meal times and amounts.
        :param T_treatment: Effective period of the treatment, float.
        :param baseline_kernels: kernel list for baseline progression, 1 kernel per patient.
        :param treatment_base_kernel: single kernel for function f_t(\tau - t_meal_j).
            Shared for the same type of treatments.
        :param mean_functions: mean functions list.
        :param patient_ids: List[int]
        :param noise_variance: variance of y
        """
        X, Y = data
        self.N = len(X)
        ...
        self.T = T_treatment
        self.separating_interval = 200  # moves input for baseline functions far so that they are not correlated
        ...
        self.ft_data = self.prepare_fa_vec(self.X_separated, self.t_separated, self.T)
        ...
        self.treatment_kc = HierarchicalMarkCoregion(output_dim=tf.shape(self.t_separated)[0], rank=1,
                                                     ...
                                                     beta0=0.1, beta1=0.1, sigma_raw=0.01,
                                                     active_dims=[1, 2])
    ...
    @staticmethod
    def prepare_fa_vec(X, t, T_treatment, Xnew=None, tnew=None):
        """
        Prepares input of the treatment response f_a:
            (i) relative time difference of meals, their dosages,
            (ii) scatter ids to write the kernel output back to the large covariance matrix Kxx (Np*Ni, Np*Ni) of
                all patients,
            (iii) scatter ids to write the kernel output back to the diagonal of the large covariance matrix Kxx.

        :param X: (N, 1)
        :param t: (Nt, 2)
        :param T_treatment: float
        :param Xnew: (N2, 1)
        :param tnew: (Nt2, 2)
        :return:
        """
        ...
        mask1 = tf.math.logical_and(deltaX1 > 0.0, deltaX1 <= T_treatment)  # 2d mask
        ...
        mask2 = tf.math.logical_and(deltaX2 > 0.0, deltaX2 <= T_treatment)
        ...
        # small perturbation to recover treatment dosages in HierarchicalMarkCoregion.K()
    ...
    def predict_ft_w_tnew(
        ...
        # Needed for Kmn, Knn
        xsnew, scatter_ids_mn, scatter_ids_nn_diag = self.prepare_fa_vec(self.X_separated, self.t_separated, self.T,
        ...
        self.treatment_kc.set_t_pidx2(tnew_porder)
        if full_cov:
            _, scatter_ids_nn, _ = self.prepare_fa_vec(Xnew_separated, tnew_separated, self.T,
                                                       Xnew=Xnew_separated, tnew=tnew_separated)
    ...
    def predict_f_w_tnew(
        ...
        self.treatment_kc.set_t_pidx(self.treatment_patient_idx)
        self.treatment_kc.set_t_pidx2(tnew_porder)
        kmn_t = self.Kt(self.X_separated, xs_train, scatter_ids_mn, Xnew_separated, xsnew)
        baseline_scatter_ids_mn = self.get_block_diag_scatter_ids(X, Xnew)
        kmn_b = self.Kb(X, scatter_ids=baseline_scatter_ids_mn, X2=Xnew)
        kmn = kmn_t + kmn_b
        self.treatment_kc.set_t_pidx(tnew_porder)
        self.treatment_kc.set_t_pidx2(tnew_porder)
        if full_cov:
            _, scatter_ids_nn, _ = self.prepare_fa_vec(Xnew_separated, tnew_separated, self.T,
                                                       Xnew=Xnew_separated, tnew=tnew_separated)
            knn_t = self.Kt(Xnew_separated, xsnew, scatter_ids_nn)
            baseline_scatter_ids_nn = self.get_block_diag_scatter_ids(Xnew, Xnew)
            knn_b = self.Kb(Xnew, baseline_scatter_ids_nn)
            knn = knn_b + knn_t
        else:
            knn_b = self.Kb_diag(Xnew)
            knn_t = self.Kt_diag(Xnew_separated, xsnew, scatter_ids_nn_diag)
            knn = knn_b + knn_t
    ...
    def predict_y_w_tnew(
        ...
        X, Y = self.data
        xs_train, scatter_ids_mm, _ = self.ft_data
        f_mean, f_var = self.predict_f_w_tnew(Xnew, tnew, tnew_porder,
                                              full_cov=full_cov, full_output_cov=full_output_cov)
        f_var = tf.reshape(tf.linalg.diag_part(f_var), (-1, 1))
        x_lengths = [tf.shape(xi)[0] for xi in Xnew]
        offsets = [tf.cast(tf.reduce_sum(x_lengths[:i]), tf.int32) for i in range(len(x_lengths))]
        mus, vars = [], []
        for xi, xl, offset, llh in zip(Xnew, x_lengths, offsets, self.likelihood):
            upper_index = offset + xl
            mu, var = llh.predict_mean_and_var(f_mean[offset:upper_index], f_var[offset:upper_index])
            mus.append(mu)
            vars.append(var)
        return tf.concat(mus, axis=0), tf.concat(vars, axis=0)
    ...
    def predict_ft_w_tnew_conditional(
        ...
        xs_train, scatter_ids_mm, _ = self.prepare_fa_vec(X_separated, t_separated, self.T)
        ...
        # Needed for Kmn, Knn
        xsnew, scatter_ids_mn, scatter_ids_nn_diag = self.prepare_fa_vec(X_separated, t_separated, self.T,
                                                                         Xnew=Xnew_separated, tnew=tnew_separated)
        self.treatment_kc.set_t_pidx(t_porder)
        self.treatment_kc.set_t_pidx2(tnew_porder)
        kmn = self.Kt(self.X_separated, xs_train, scatter_ids_mn, Xnew_separated, xsnew)
    ...
    def predict_ft_single(
        ...
        self.treatment_kc.set_t_pidx2(tnew_pidx)
        if full_cov:
            _, scatter_ids_nn, _ = self.prepare_fa_vec(Xnew, tnew, self.T, Xnew=Xnew, tnew=tnew)
