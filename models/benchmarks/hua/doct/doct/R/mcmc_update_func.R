...
calculate_EY_no_cov<-function(beta_l,b_il,D,Y,Ts,Npat,inds_all){
  n<-length(Y)
  EY<-mean_fixed<-mean_rand<-numeric(n)
  for (i in 1:Npat){
    inds<-inds_all[[i]]
    ...
    cov_il<-cbind(1,D[c(inds_less[1],inds_less)],Ts[inds])
    cov_l<-cbind(1,D[c(inds_less[1],inds_less)],Ts[inds],Ts[inds]^2)
    mean_fixed[inds]<-t(beta_l)%*%t(cov_l)
    mean_rand[inds]<-t(b_il[,i])%*%t(cov_il)
    # mean_rand[inds[-1]]<-b_il[1,i]
    EY[inds]<-mean_fixed[inds]+mean_rand[inds]
...
update_beta_l_separate_no_cov<-function(beta_l,beta_d,beta_s,sigma2_l,sigma2_d,D,Y,Ts,id,Npat,int_all,h0,b_il,mean_rand,B,
                                 shape,censor,theta_a,beta_alpha,death_init_death,end_inds,inds_shrink_both,
                                 inds_shrink_init,inds_shrink_end,beta_sd,
                                 beta_sd_cum,Toxicity,lambda_tox,b_il3,inds_all,k,Ncov_l,death_time,J_cumsum, J,accept_ind=F){
  curr_beta_l<-beta_l
  cov_all<-cbind(1,D[inds_shrink_end],Ts[inds_shrink_init],Ts[inds_shrink_init]^2)
  Sigma<-chol2inv(chol(diag(0.01^2,Ncov_l)+t(cov_all)%*%(cov_all)/sigma2_l))
  mu<- 1/sigma2_l*Sigma%*%t(cov_all)%*%(Y[inds_shrink_init]-mean_rand[inds_shrink_init])
  prop_beta_l<-mvrnorm(n=1,mu=mu,Sigma=Sigma)
  accept_fixed<-0
  prop_out<-calculate_EY_no_cov(prop_beta_l,b_il,D,Y,Ts,Npat,inds_all)
  prop_EY<-prop_out$EY
  prop_mean_fixed<-prop_out$fixed
  curr_beta_l<-prop_beta_l
  accept_fixed<-accept_fixed+1
...
update_beta_d_no_cov<-function(beta_d,sigma2_d,Y,D,Ts,id,int_all,Npat){
  cov_all<-c()
  d_all<-c()
  Ncov<-length(beta_d)
  cov_all<-cbind(1,Y)
  d_all<-D
  cov_matrix = diag(0.01^2,Ncov)+t(cov_all)%*%(cov_all)/sigma2_d
  Lc = chol(cov_matrix) 
  Sigma<-chol2inv(Lc)
  mu<- 1/sigma2_d*Sigma%*%t(cov_all)%*%d_all
  new_beta_d<-mvrnorm(n=1,mu=mu,Sigma=Sigma)
...
# init_beta_l_no_cov(Ncov_l,0.5^2,D,Y,Ts,id,Npat)
init_beta_l_no_cov<-function(Ncov,sigma2_l,D,Y,Ts,id,Npat){
  cov_all<-c()
  y_all<-c()
  for (i in 1:Npat){
    inds<-which(id==i)
    ...
    time<-Ts[inds]
    if (len > 1) {
      cov_l<-cbind(1,d_i[-len],time[-1],time[-1]^2)
      cov_all<-rbind(cov_all,cov_l)
      y_all<-c(y_all,y_i[-1])
    }
  }
...
update_theta_a_no_survival<-function(beta_v,Y,Ts,mu,theta_a,Npat,jump_theta_a,beta_alpha,beta_s,death_time,beta_l,D,id,h0,mean_rand,EY,shape,
                         censor,inds_shrink_init,inds_shrink_end,death_init_death,end_inds,beta_sd,beta_sd_cum,
                         Toxicity,lambda_tox,inds_shrink_both,b_il3,curr_loglik_surv=NA,k,Ncov_l,peak_dist){
  cur_theta_a<-theta_a
  accept<-c(0,0)
...
update_k_no_survival<-function(beta_v,Y,Ts,mu,theta_a,Npat,jump_theta_a,beta_alpha,beta_s,death_time,beta_l,D,id,h0,mean_rand,EY,shape,
                   censor,inds_shrink_init,inds_shrink_end,death_init_death,end_inds,beta_sd,beta_sd_cum,Toxicity,
                   lambda_tox,inds_shrink_both,b_il3,curr_loglik_surv=NA,k,jump_k,a=400,b=200,Ncov_l,peak_dist){
  cur_k<-k
  accept<-0
...
update_sigma2_d_no_cov<-function(lambda1_d,lambda2_d,Y,Npat,id,int_all,beta_d,B,D){
  n<-length(Y)-Npat
  cov_d<-cbind(1,Y)
  d_sum_prod = beta_d%*%t(cov_d)
  sum=(rbind(D)-d_sum_prod)%*%t(rbind(D)-d_sum_prod)/2
  lambda1_d_post<-lambda1_d+n/2
  lambda2_d_post<-lambda2_d+sum
  new_sigma2_d<-1/rgamma(1,shape=lambda1_d_post,rate=lambda2_d_post)
  return(new_sigma2_d)
